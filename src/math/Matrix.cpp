...
Matrix::Matrix(unsigned int r, unsigned int c, long double v) {
	this->r = r;
	this->c = c;
	this->arr = new long double[r * c];
...
//Array constructor
Matrix::Matrix(int r, int c, long double* arr) {
	this->r = r;
	this->c = c;
	this->arr = arr;
}
...
//get values
long double Matrix::get(unsigned int row, unsigned int col) {
	//Catch out of bounds
...
//set
void Matrix::set(unsigned int row, unsigned int col, long double v) {
	//Catch out of bounds
	if (row < 0 || col < 0 || row >= r || col >= c) {
		string vals = "Index " + to_string(row) + ',' + to_string(col)
				+ " called on matrix size of " + to_string(r) + ','
...
void Matrix::transpose() {
	long double* tr = new long double[r * c];
...
Matrix Matrix::operator*(const Matrix& m) const {
	...
	long double *prod = new long double[r * m.c];
	for (unsigned int i = 0; i < r; i++) {
...
void Matrix::apply(long double (*func)(long double)) {
	for (unsigned int i = 0; i < r * c; i++) {
		arr[i] = func(arr[i]);
	}
}
...
// Does the matrix contain a bad value?
bool containsNaN(Matrix& m) {
	for (unsigned int x = 0; x < m.getr(); x++) {
		for (unsigned int y = 0; x < m.getr(); x++) {
			if (isnan(m.get(x, y))) {
				return true;
			}
		}
