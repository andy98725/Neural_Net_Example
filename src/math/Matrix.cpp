...
Matrix::Matrix(unsigned int r, unsigned int c, double v) {
	this->r = r;
	this->c = c;
...
//set
void Matrix::set(unsigned int row, unsigned int col, double v) {
	//Catch out of bounds
	if (row < 0 || col < 0 || row >= r || col >= c) {
		string vals = "Index " + to_string(row) + ',' + to_string(col)
				+ " called on matrix size of " + to_string(r) + ','
